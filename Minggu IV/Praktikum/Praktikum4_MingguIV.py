...
def comprehensive_enhancement_pipeline(image):
    ...
    final = cv2.normalize(local_enhanced, None, 0, 255, cv2.NORM_MINMAX)
    results['final'] = final
...
for step_name, img in steps:
    metrics = analyze_image_statistics(img, step_name)
    print(f"{step_name:<25} {metrics['mean']:<10.2f} {metrics['std']:<10.2f}"
          f"{metrics['min']:<10.2f} {metrics['max']:<10.2f} {metrics['entropy']:<10.4f}")
