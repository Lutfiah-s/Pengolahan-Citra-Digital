...
def demonstrate_image_registration():
    """Demonstrate image registration using feature matching + affine transform"""
    ...
    # -------------------------------------------------
    # 2. Create transformed (moving) image
    # -------------------------------------------------
    M_true = cv2.getRotationMatrix2D((128, 128), 15, 0.9)
    M_true[0, 2] += 20  # translation x
    M_true[1, 2] += 15  # translation y
    ...
    # -------------------------------------------------
    M_estimated, inliers = cv2.estimateAffinePartial2D(pts_mov, pts_ref)
    ...
    axes[0,1].set_title("Moving Image\n(Rot+Scale+Trans+Noise)")
    axes[0,1].axis("off")

    axes[0,2].imshow(registered_img, cmap='gray')
    axes[0,2].set_title("Registered Image")
    axes[0,2].axis("off")

    # Row 2
    diff_before = cv2.absdiff(ref_img, moving_img)
    diff_after = cv2.absdiff(ref_img, registered_img)
    ...
    match_img = cv2.drawMatches(ref_img, kp1, moving_img, kp2,
                                good_matches[:30], None, flags=2)

    axes[1,2].imshow(match_img)
    axes[1,2].set_title("Feature Matches (Top 30)")
    axes[1,2].axis("off")

    plt.suptitle("Image Registration using ORB + Affine Transform",
    ...
    print(np.round(M_estimated, 3))

    print(f"\nMSE Before Registration : {mse_before:.2f}")
    print(f"MSE After Registration  : {mse_after:.2f}")

    return registered_img


# Jalankan demonstrasi
registered_result = demonstrate_image_registration()