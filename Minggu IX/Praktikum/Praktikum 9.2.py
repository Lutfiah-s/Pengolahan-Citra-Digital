...
def praktikum_9_2():
    ...
    def apply_prewitt_edge_detection(image):
        ...
        prewittx = cv2.filter2D(image.astype(np.float32), cv2.CV_32F, kernelx)
        prewitty = cv2.filter2D(image.astype(np.float32), cv2.CV_32F, kernely)
    ...
    def region_growing_segmentation(image, seeds, threshold=20):
        ...
        for seed in seeds:
            ...
            while stack:
                ...
                visited[x, y] = True
                current_value = int(image[x, y])
                region_sum += current_value
                region_count += 1
                segmented[x, y] = 255
                ...
                for nx, ny in neighbors:
                    if (0 <= nx < image.shape[0] and 0 <= ny < image.shape[1] 
                        ...
                        neighbor_value = int(image[nx, ny])
    ...
    # Sobel direction (visualized with HSV colormap)
    axes[1, 1].imshow(sobel_direction, cmap='hsv', vmin=-180, vmax=180)
    axes[1, 1].set_title('Sobel Edge Direction\n(Gradient Angle)')
    axes[1, 1].axis('off')
    
    # Combined edges visualization
    combined_edges = np.maximum(edge_results['Sobel'], edge_results['Canny'])
